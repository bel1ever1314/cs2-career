...
const ui = {pages:{},head:()=>'',tabs:()=>'',link:()=>'',paint:(host,html)=>host.innerHTML=html};
...
(async()=>{
  ...
  const positions=[...host.innerHTML.matchAll(/class="graph-position" data-ui-key="[^"]+" style="left:(\d+)px;top:([\d.]+)px"/g)]
