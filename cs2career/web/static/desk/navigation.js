...
window.CareerUI = (() => {
  ...
  const labels = {home:'生涯概览',squad:'我的阵容',honours:'荣誉墙',locker:'俱乐部经营',mail:'收件箱',market:'转会市场',
    ...
    profile:'资料中心',inventory:'饰品库存',skins:'饰品市场',cases:'武器箱',setup:'创建生涯'};
  function remember() {
    if (cursor < 0) return;
  ...
  function draw() {
    ...
    const painter=pages[VIEW];
    if(!painter) {queueMicrotask(restore);return false;}
    const stamp=++generation, host=$('view-'+VIEW);
    host.innerHTML='<p class="empty">正在读取本地资料…</p>';
    Promise.resolve(painter({...route},host,()=>stamp===generation&&route.view===VIEW)).then(()=>{
      if(stamp!==generation)return;restore();
    }).catch(err=>{if(stamp===generation)host.innerHTML=`<p class="empty">资料暂不可用：${esc(err.message)}</p>`;});
    return true;
  }
  ...
  document.addEventListener('DOMContentLoaded',()=>{
    ...
    document.addEventListener('keydown',e=>{if(e.altKey&&['ArrowLeft','ArrowRight'].includes(e.key)){e.preventDefault();travel(e.key==='ArrowLeft'?-1:1);}});
  });
