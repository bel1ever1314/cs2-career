...
3. `py -3 build_desktop_preview.py --integration --name CS2Career-AssistChat`。
   这是独立窗口程序，默认不会打开浏览器。构建不会扫描或复制玩家 save。
4. `py -3 tools/package_public.py --exe release/CS2Career-AssistChat/CS2Career-AssistChat.exe`。
   默认输出到 publish/1.5.0-assistchat.1；输出已存在会停止，避免覆盖。可用 --output 指定新目录。
