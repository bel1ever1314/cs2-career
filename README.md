...
Windows 上的 CS2 生涯模拟器。当前版本 **1.4.1**。用 Python 3 即可运行，不需要再装 Python 包。
...
生涯是外挂在 CS2 外面的模拟器。如果进入cs2游玩请在启动项添加-insecure
...
- 不装：不能进 CS2 自己打，但仍然可用当前游戏内培养数值完成比赛
在 **训练赛** 页填好并保存：
...
- 重置了游戏内的数值系统；转会市场重分档，年底青训补人。
- 重置了游戏内的rating算法。
- 修改了一些问题。
- 新增了一些剧情。

发布包是 GitHub Release 里的 `CS2Career-1.4.1.zip`（内含 `CS2Career.exe` 和说明书）。